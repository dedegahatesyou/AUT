...
Shadow.ZIndex = -5 Shadow.Name = "Shadow" 
...
local function NewPage(ButtonImage, PageTitle) 
  ...
  Underline.ZIndex = -2 Underline.Name = "Line" 
...
local function NewToggle(Page, Name, Callback) 
  ...
  Button.MouseButton1Click:Connect(function() 
      ...
      if ToggleCount % 2 == 1 then 
        -- True 
        local Tween = TS:Create(Button, TweenInfo.new(0.3), { ImageTransparency = 0 }) Tween:play() Callback(ToggleCount % 2) 
      else 
        -- False 
        local Tween = TS:Create(Button, TweenInfo.new(0.3), { ImageTransparency = 1 }) Tween:play() Callback(ToggleCount % 2) 
      end 
    end) 
end 
...
return { ["NewPage"] = NewPage, ["NewToggle"] = NewToggle, ["InitUI"] = InitUI, ["SetMain"] = SetMain }
